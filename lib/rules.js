...
/**
 * Reduce one rendered snapshot to the text that decides re-injection: the frame
 * with its intro's matched-file list removed.
 *
 * The list records which files had matched when the snapshot was rendered. It is
 * informational, so a later step that only adds matched files must not re-send
 * every active rule body: injections are keyed on the active rule set and its
 * content. The frame's second line is the intro; its list variant, its list-free
 * variant, and a version cut short by the byte budget all reduce to the same
 * canonical intro.
 *
 * @param text - rendered snapshot text, or the empty-set sentinel.
 * @returns the text with the intro line reduced to its list-free form.
 */
export function snapshotKey(text) {
	const lines = text.split("\n");
	if (lines.length < 2 || !isSnapshotIntro(lines[1])) return text;
	lines[1] = RULES_INTRO;
	return lines.join("\n");
}

/** Whether a frame's second line opens the intro, in any variant or truncation. */
function isSnapshotIntro(line) {
	return line.length > 0 && (RULES_INTRO.startsWith(line) || line.startsWith(RULES_INTRO));
}
