...
var RulesState = class {
	...
	* Returns `undefined` when nothing should be injected: the active rule set and
	* its content are unchanged (the matched-file list inside a snapshot is
	* informational and never re-injects on its own), or nothing is active and
	* nothing was injected before.
	...
	*/
	async compose(agent, signal) {
		...
		// Compare list-free keys: a step that only added matched files renders a
		// different text without changing any rule, and re-sending every active
		// body would spend context for nothing. The injected text keeps the list
		// as rendered, and remains the payload a resumed session restores from.
		if (typeof previous === "string" && snapshotKey(previous) === snapshotKey(desiredText)) return void 0;
