...
- **注入点**：`agent/pre-step` 瀑布监听器追加一条 `<rules>` 框架的用户消息；只在激活规则集合或规则正文变化时追加新消息——快照里的 `matched files` 列表只是渲染当时的记录，仅新增匹配文件不会重发规则正文。
...
- 快照里的 `matched files` 列表是渲染当时的记录：之后新增的匹配文件不会重发规则，所以模型手里那份清单可能滞后；激活集合始终以最新快照为准。
